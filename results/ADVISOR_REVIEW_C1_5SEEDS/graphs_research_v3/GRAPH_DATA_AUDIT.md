...
| 3 | What active behavior occupied time? | swarm_trajectory: sim_time_s, scout_id, phase | run; consecutive-sample phase durations up to each Scout/run depletion endpoint | trajectory sample resolution is 0.1 s |
...
All requested behavioral duration categories that exist in the trajectory log were reconstructed. `APPROACH_RESOURCE` and a separate energy-collection-start event are not logged; neither is inferred or shown as a separate category.
